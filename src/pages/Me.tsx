import { ArrowRight, CarFront, Edit3, Map, MessageSquareText, Route, Sparkles, UserRound } from 'lucide-react'
...
export function Me() {
  ...
  return <AppShell><div className="passport-page page-frame">
    <header className="passport-hero"><div><span>MY MOBILITY PASSPORT</span><h1>{state.user.name} 的汽车生活护照</h1><p>不是一张能力证明，而是 FirstDrive 理解你、车、路与环境的共同上下文。</p><button className="passport-reintake" onClick={() => navigate('/welcome')}><MessageSquareText />重新用一句话介绍自己</button></div><aside><small>PROFILE UPDATED</small><strong>{state.memory.person.lastUpdated}</strong><span>{profile.city} · v5.0</span></aside></header>
